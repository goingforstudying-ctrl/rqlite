...
// Test_StashPopFiles verifies that StashFiles moves every present database,
// WAL, and SHM file to the stash paths, that PopFiles restores them with
// their contents intact, and that popping with no stash present is a no-op.
// Each test case lists which of the three files exist.
...
// assertStashFileContents fails the test if the file at path does not contain
// exactly want.
